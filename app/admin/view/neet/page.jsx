...
const NEET = () => {
  ...
  const fetchFirebaseItems = async (isLoadMore = false) => {
    try {
      ...
      let itemsQuery = query(collection(db, 'neet'), limit(itemLimit));

      if (lastDoc && isLoadMore) {
        itemsQuery = query(collection(db, 'neet'), startAfter(lastDoc), limit(itemLimit));
      }
  ...
  const handleDelete = async (id) => {
    try {
      const db = getFirestore(app);
      await deleteDoc(doc(db, 'neet', id));
